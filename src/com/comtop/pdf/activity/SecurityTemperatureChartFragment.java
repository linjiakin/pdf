 ...
 * @author linyong
 */
public class SecurityTemperatureChartFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		return inflater.inflate(R.layout.security_temperature_chart, container,false);
	}
	...
	private void init() {
		//查询时间
		queryTime = (EditText) getActivity().findViewById(R.id.security_temperature_chart_querytime_id);
		...
		//查询按钮
		query = (Button) getActivity().findViewById(R.id.security_temperature_chart_query_id);
		query.setOnClickListener(clickListener);
		
		//加载图表
		webView = (WebView) getActivity().findViewById(R.id.security_temperature_chart_webview_id);
	...
	OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			switch (v.getId()) {
			case R.id.security_temperature_chart_querytime_id:
				showDateDialog(queryTime);
				break;
			case R.id.security_temperature_chart_query_id:
				webView.loadUrl(Constants.SECURITY_TEMPERATURE_CHART_URL+"?powerroomId="+
						getHouseFunctionId()+"&queryTime="+queryTime.getText());
				break;
			default:
				break;
			}
